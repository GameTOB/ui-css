<!-- page-title -->
<div class="doc-row page-title clearfix content">
	<div class="pull-left">
		<!-- tit -->
		<div class="title-base">
			<span class="title-icon"></span>
			<span class="title-name">模块标题</span>
			<small>小标题或描述性文字</small>
		</div>
		<!-- /tit -->
	</div>
	<div class="pull-right search">
		<span class="input-icon"> <input type="text" placeholder="Search ..." class="input-search" /> <i class="fa fa-search"></i>
		</span>
	</div>
</div>
<!-- /page-title -->
<h5><span class="label label-warning">简单面板</span></h5>
<div class="panel panel-default">
	<div class="panel-body">
		基本模块
	</div>
</div>
<div class="panel panel-default">
	<div class="panel-body">
		内容区
	</div>
	<div class="panel-footer">尾</div>
</div>
<h5><span class="label label-warning">带图片内容的面板</span></h5>
<!-- panel -->
<div class="panel panel-default">
	<div class="panel-heading clearfix">
		面板名称
		<small>其它文字</small>
		<a href="#" class="pull-right">更多</a>
	</div>
	<div class="panel-body element-con">
		<div class="">
			<div class="col-xs-6 col-md-3">
				<a href="#" class="thumbnail">
					<img src="static/img/defaultimage.png" alt="default image">
				</a>
			</div>
			<div class="col-xs-6 col-md-3">
				<a href="#" class="thumbnail">
					<img src="static/img/defaultimage.png" alt="default image">
				</a>
			</div>
			<div class="col-xs-6 col-md-3">
				<a href="#" class="thumbnail">
					<img src="static/img/defaultimage.png" alt="default image">
				</a>
			</div>
			<div class="col-xs-6 col-md-3">
				<a href="#" class="thumbnail">
					<img src="static/img/defaultimage.png" alt="default image">
				</a>
			</div>
		</div>
	</div>
</div>
<!--/panel -->
<h5><span class="label label-warning">内容为筛选条件容器</span></h5>
<!-- panel -->
<div class="panel  panel-primary">
	<div class="panel-heading">
		面板名称
	</div>
	<div class="panel-body element-con clearfix">
		<!-- form-->
		<form class="form-inline">
			<div class="form-group form-margin-base col-xs-3">
				<label for="exampleInputName2">标签名称：</label>
				<input type="text" class="form-control" id="exampleInputName2" placeholder="请输入" />
			</div>
			<div class="form-group form-margin-base col-xs-3">
				<label>标签名称：</label>
				<select class="form-control"> <option>select1</option> <option>select2</option> <option>select3</option> <option>select4</option> <option>select5</option> </select>
			</div>
			<div class="form-group form-margin-base col-xs-3">
				<label>标签名称：</label>
				<select class="form-control"> <option>select1</option> <option>select2</option> <option>select3</option> <option>select4</option> <option>select5</option> </select>
			</div>
			<div class="form-group form-margin-base col-xs-3">
				<label>标签名称：</label>
				<select class="form-control"> <option>select1</option> <option>select2</option> <option>select3</option> <option>select4</option> <option>select5</option> </select>
			</div>
			<div class="form-group form-margin-base col-xs-3">
				<label>标签名称：</label>
				<select class="form-control"> <option>select1</option> <option>select2</option> <option>select3</option> <option>select4</option> <option>select5</option> </select>
			</div>
			<div class="form-group form-margin-base col-xs-3">
				<label>标签名称：</label>
				<select class="form-control"> <option>select1</option> <option>select2</option> <option>select3</option> <option>select4</option> <option>select5</option> </select>
			</div>
			<div class="form-group form-margin-base col-xs-3">
				<label>标签名称：</label>
				<input type="text" class="form-control" id="exampleInputName2" placeholder="请输入" />
			</div>
			<div class="form-group form-margin-base col-xs-3">
				<label>标签名称：</label>
				<input type="text" class="form-control" id="exampleInputName2" placeholder="请输入" />
			</div>
			<div class="clearfix"></div>
			<div class="btn-group pull-right">
				<button type="button" class="btn btn-primary"><i class="fa fa-search"></i>检索</button>
				<button type="button" class="btn btn-default"><i class="fa fa-refresh"></i>重置</button>
			</div>
		</form>
		<!--/form-->
	</div>
</div>
<!--/panel -->
<h5><span class="label label-warning">默认用法</span></h5>
<!-- panel -->
<div class="panel panel-danger ">
	<div class="panel-heading clearfix">
		面板名称
		<small>其它文字</small>
		<a href="#" class="pull-right">更多 <span class="badge">42</span></a>
	</div>
	<div class="panel-body element-con">
		内容显示区
	</div>
</div>
<!--/panel -->

<div class="doc-row clearfix">
	
	<div class="col-xs-4">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h3 class="panel-title">Panel primary</h3>
			</div>
			<div class="panel-body">
				Panel content
			</div>
		</div>
		<div class="panel panel-success">
			<div class="panel-heading">
				<h3 class="panel-title">Panel success</h3>
			</div>
			<div class="panel-body">
				Panel content
			</div>
		</div>
		<div class="panel panel-warning">
			<div class="panel-heading">
				<h3 class="panel-title">Panel warning</h3>
			</div>
			<div class="panel-body">
				Panel content
			</div>
		</div>
	</div>

	<div class="col-xs-4">
		<div class="panel panel-danger">
			<div class="panel-heading">
				<h3 class="panel-title">Panel danger</h3>
			</div>
			<div class="panel-body">
				Panel content
			</div>
		</div>
		<div class="panel panel-info">
			<div class="panel-heading">
				<h3 class="panel-title">Panel info</h3>
			</div>
			<div class="panel-body">
				Panel content
			</div>
		</div>
	</div>

</div>