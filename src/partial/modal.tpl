<!-- content -->
<div class="content">
   <!-- modal -->
   <!-- 模态框（Modal） -->
   <div class="modal" style="display:inherit;left:auto">
      <div class="modal-dialog">
         <div class="modal-content">
            <div class="modal-header">
               <button type="button" class="close"
               >
               &times;
               </button>
               <h4 class="modal-title">
               模态框（Modal）标题
               </h4>
            </div>
            <div class="modal-body">
               这里显示文本
            </div>
            <div class="modal-footer clearfix">
               <button type="button" class="btn btn-default"
               >关闭
               </button>
               <button type="button" class="btn btn-primary">
               提交更改
               </button>
            </div>
         </div>
      </div>
   </div>
   <!-- /modal -->
   <!-- modal -->
   <!-- 模态框（Modal） -->
   <div class="modal" style="display:inherit; margin-top:10%;left:auto">
      <div class="modal-dialog">
         <div class="modal-content">
            <div class="modal-header">
               <button type="button" class="close"
               >
               &times;
               </button>
               <h4 class="modal-title">
               模态框（Modal）标题
               </h4>
            </div>
            <div class="modal-body clearfix">
               <!-- form-->
               <form class="form-inline clearfix">
                  <div class="form-group form-margin-base col-xs-12">
                     <label  class="control-label">标签名称：</label>
                     <input type="text" class="form-control"  placeholder="请输入" />
                  </div>
                  <div class="form-group form-margin-base col-xs-12">
                     <label  class="control-label">标签名称：</label>
                     <input type="text" class="form-control"  placeholder="请输入" />
                  </div>
                  <div class="form-group form-margin-base col-xs-12">
                     <label  class="control-label">标签名称：</label>
                     <label class="radio-inline">
                        <input type="radio"> 全部
                     </label>
                     <label class="radio-inline">
                        <input type="radio"> 指定
                     </label>
                     <input type="text" class="form-control"  placeholder="请输入" />
                  </div>
                  <div class="clearfix"></div>
                  <div class="modal-footer">
                     <button type="button" class="btn btn-default"
                     >关闭
                     </button>
                     <button type="button" class="btn btn-primary">
                     提交更改
                     </button>
                  </div>
               </form>
               <!--/form-->
            </div>
         </div>
      </div>
   </div>
   <!-- /modal -->
   <!-- modal -->
   <!-- 模态框（Modal） -->
   <div class="modal" style="display:inherit; margin-top:26%;left:auto">
      <div class="modal-dialog">
         <div class="modal-content">
            <div class="modal-header">
               <button type="button" class="close"
               >
               &times;
               </button>
               <h4 class="modal-title">
               模态框（Modal）标题
               </h4>
            </div>
            <div class="modal-body clearfix">
               <!-- form-->
               <form class="form-inline clearfix">
                  <div class="form-group form-margin-base col-xs-12">
                     <label  class="control-label">标签名称：</label>
                     <button type="button" class="btn btn-fail btn-sm">不可点击</button>
                     <button type="button" class="btn btn-success btn-sm">被选选项</button>
                     <input type="text" class="form-control"  placeholder="请输入" />
                  </div>
                  <div class="form-group form-margin-base col-xs-12">
                     <label  class="control-label">标签名称：</label>
                     <input type="text" class="form-control"  placeholder="请输入" />
                  </div>
                  <div class="form-group form-margin-base col-xs-12">
                     <label  class="control-label">标签名称：</label>
                     <label class="radio-inline">
                        <input type="radio"> 全部
                     </label>
                     <label class="radio-inline">
                        <input type="radio"> 指定
                     </label>
                  </div>
                  <div class="form-group form-margin-base col-xs-12">
                     <label  class="control-label">标签名称：</label>
                     <input type="text" class="form-control" placeholder="请输入" />
                  </div>
                  <div class="form-group form-margin-base col-xs-12">
                     <label  class="control-label">标签名称：</label>
                     <input type="text" class="form-control" placeholder="请输入" />
                  </div>
                  <div class="form-group form-margin-base col-xs-12">
                     <label  class="control-label">标签名称：</label>
                     <label class="checkbox-inline">
                        <input type="checkbox"  value="option1"> 1
                     </label>
                     <label class="checkbox-inline">
                        <input type="checkbox" value="option2"> 2
                     </label>
                     <label class="checkbox-inline">
                        <input type="checkbox" value="option3"> 3
                     </label>
                  </div>
                  <div class="clearfix"></div>
                  <div class="modal-footer">
                     <button type="button" class="btn btn-default"
                     >关闭
                     </button>
                     <button type="button" class="btn btn-primary">
                     提交更改
                     </button>
                  </div>
               </form>
               <!--/form-->
            </div>
         </div>
      </div>
   </div>
   <!-- /modal -->
   <!-- modal -->
   <!-- 模态框（Modal） -->
   <div class="modal ui-vertical-display" style="display:block">
      <div class="modal-lg">
         <div class="modal-content">
            <div class="modal-header">
               <button type="button" class="close">&times;</button>
               <h4 class="modal-title">
               模态框（Modal）标题
               </h4>
            </div>
            <div class="modal-body clearfix">
               <!-- form-->
               <form class="form-inline clearfix">
                  <div class="form-group form-margin-base col-xs-6">
                     <label class="control-label">标签名称：</label>
                     <input type="text" class="form-control"  placeholder="请输入" />
                  </div>
                  <div class="form-group form-margin-base col-xs-6">
                     <label class="control-label"><mark class="text-danger">*</mark> 标签名称：</label>
                     <input type="text" class="form-control"  placeholder="请输入" />
                  </div>
                  <div class="form-group form-margin-base col-xs-12">
                     <label class="control-label">较长标签名称：</label>
                     <label class="radio-inline">
                        <input type="radio"> 全部
                     </label>
                     <label class="radio-inline">
                        <input type="radio"> 指定
                     </label>
                     <input type="text" class="form-control"  placeholder="请输入" />
                  </div>
                  <div class="form-group form-margin-base modal-scroll col-xs-12">
                     <button type="button" class="btn btn-default btn-sm">默认选项</button>
                     <button type="button" class="btn btn-success btn-sm">被选选项</button>
                     <button type="button" class="btn btn-default btn-sm">默认选项</button>
                     <button type="button" class="btn btn-success btn-sm">被选选项</button>
                     <button type="button" class="btn btn-default btn-sm">默认选项</button>
                  </div>
                  <div class="clearfix"></div>
                  <div class="modal-footer">
                     <button type="button" class="btn btn-default">关闭</button>
                     <button type="button" class="btn btn-primary">提交更改</button>
                  </div>
               </form>
               <!--/form-->
            </div>
         </div>
      </div>
   </div>
   <!-- /modal -->
</div>
